<footer class="py-8 mt-16">
    <div class="flex md:flex-row flex-col justify-center p-4 custom-align-center">
        <b>
            Thanks for using Memehub!
        </b>
        <img src="{{ asset('img/heart.svg') }}" alt="Thank you!" style="max-width: 350px">
    </div>
{{--    <div class="py-3 flex justify-center custom-align-center">--}}
{{--        <b class="text-md">--}}
{{--            Meme Hub | follow us in social.--}}
{{--        </b>--}}
{{--        <div>--}}
{{--            <a href="#"><i class="fab fa-instagram mx-2"></i></a>--}}
{{--            <a href="#"><i class="fab fa-twitter mx-2"></i></a>--}}
{{--            <a href="#"><i class="fab fa-facebook mx-2"></i></a>--}}
{{--        </div>--}}
{{--    </div>--}}
    <div class="py-3 mt-4 flex justify-center custom-align-center">
        <b class="text-md">
            © MemeHub 2021
        </b>
    </div>
</footer>
