    ...
    <div class="w-full text-gray-700 bg-white shadow-sm">
        <div x-data="{ open: false }" class="flex flex-col max-w-screen-xl px-4 mx-auto md:items-center md:justify-between md:flex-row md:px-6 lg:px-8">
            <div class="p-4 flex flex-row items-center justify-between">
                <a href="{{ route('home') }}" class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg focus:outline-none focus:shadow-outline">Meme Hub</a>
            ...
            <nav :class="{'flex': open, 'hidden': !open}" class="flex-col flex-grow pb-4 md:pb-0 hidden md:flex md:justify-end md:flex-row">
                <a class="px-4 py-2 mt-2 text-sm font-semibold @if(Route::currentRouteName() == "home") text-gray-900 bg-gray-200 @else bg-transparent @endif rounded-lg md:mt-0 md:ml-2 ml-0 hover:text-gray-900 focus:text-gray-900 transition duration-200 ease-in-out hover:underline focus:bg-gray-100 focus:outline-none focus:shadow-outline" href="{{ route('home') }}">Home</a>
                <a class="px-4 py-2 mt-2 text-sm font-semibold @if(Route::currentRouteName() == "memes.index") text-gray-900 bg-gray-200 @else bg-transparent @endif rounded-lg md:mt-0 md:ml-2 ml-0 hover:text-gray-900 focus:text-gray-900 transition duration-200 ease-in-out hover:underline focus:bg-gray-100 focus:outline-none focus:shadow-outline" href="{{ route('memes.index') }}">Memes</a>
                <div @click.away="open = false" class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48" style="z-index: 9">
                        <div class="px-2 py-2 bg-white rounded-md shadow">
                            @forelse ($g_categories as $category)
                            ...
                            @empty
                                <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg md:mt-0 hover:text-gray-900 focus:text-gray-900 transition duration-200 ease-in-out focus:bg-gray-100 focus:outline-none focus:shadow-outline" disabled>No categories</a>
                            @endforelse
                        </div>
                    </div>
                </div>
